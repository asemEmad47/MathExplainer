...
public class CharacterProbs : MonoBehaviour
{
    ...
    public static void CenterInPos(float XPos, float YPos, ref GameObject obj, TextMeshProUGUI FirstNumPlace)
    {
        ...
        else if (obj.TryGetComponent<Transform>(out var transform))
        {
            ...
            {
                Vector3 worldPosition = parentRectTransform.TransformPoint(new Vector3(XPos, YPos, 1000));
                transform.position = worldPosition;
            }
            else
            {
                Debug.LogWarning("Parent is not a RectTransform. Using world position directly.");
                transform.position = FirstNumPlace.transform.position + new Vector3(XPos, YPos, 1000);
            }
        }
        else
        {
