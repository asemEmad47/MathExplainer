...
public class AddingTwoFractionsScript : MonoBehaviour
{
    ...
    private int finalRes = 0;
    private TextMeshProUGUI NonSimplifiedNue;
    private TextMeshProUGUI NonSimplifiedDeno;


    List<TMP_InputField> FieldsList;
    public void Start()
    {
        GCFScript.IsCalledFromOutside = true;
    ...
    void Update()
    {
       ...
       if (IsCalledFromOutSide && FNue.text.Length != 0 && SNue.text.Length != 0 && FDeno.text.Length != 0 && SDeno.text.Length != 0 && int.Parse(FNue.text) >= int.Parse(SNue.text))
        {
            WarningMessage.GetComponent<TextMeshProUGUI>().enabled = false;
    ...
    public IEnumerator StartExplaining()
    {
        ...
        else
        {
            ...
            if(!IsCalledFromOutSide) 
                ...
                yield return (StartCoroutine(SLStaicFunctions.PlayByAddress(this, "so start subtracting" + SpeakerName, IsExplain)));
        ...
        YPos -= 200;
        yield return (StartCoroutine(SLStaicFunctions.PlayByAddress(this, "now" + SpeakerName, IsExplain)));
        ...
        PrimeFactors.branches = 1;

        yield return StartCoroutine(DoOperation(FNueRes, SNueRes, FNumNewNum, SNumNewNum, XPos, YPos));

        for (int i = Math.Abs(Math.Min(finalRes, int.Parse(LCMRes))); i >= 2; i--)
        {
            if (finalRes % i == 0 && int.Parse(LCMRes) % i == 0)
            {
                ...
                Debug.Log(NonSimplifiedNue.text +" "+NonSimplifiedDeno.text);
    ...
    public IEnumerator DoOperation( TextMeshProUGUI FNueRes , TextMeshProUGUI SNueRes , int FNumNewNum , int SNumNewNum , float XPos , float YPos)
    {
        ...
        if (!IsCalledFromOutSide)
        {
            ...
            finalRes = FNumNewNum + SNumNewNum;
        ...
        {
            yield return (StartCoroutine(SLStaicFunctions.PlayByAddress(this, "minus" + SpeakerName, IsExplain)));
        ...
        NonSimplifiedNue = GameObject.Find((123).ToString()).GetComponent<TextMeshProUGUI>();

        NonSimplifiedDeno = GameObject.Find((-444).ToString()).GetComponent<TextMeshProUGUI>();
    }
