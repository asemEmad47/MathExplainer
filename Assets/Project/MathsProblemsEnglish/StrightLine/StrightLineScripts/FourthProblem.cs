...
public class FourthProblem : MonoBehaviour
{
    ...
    public IEnumerator GetLineOneSol()
    {
        ...
        if (SlopeDeno == 1 || OldNue !=SlopeNue)
        {
            yield return (StartCoroutine(SLStaicFunctions.PlayByAddress(this, "equal" + SpeakerName, Explain)));
    ...
    public IEnumerator GetLineTwoSol()
    {
        ...
        TextInstantiator.InstantiateText(FirstNumPlace, "since L<sub>1</sub>//L<sub>2</sub>", XPos + 115, Ypos, 0, false);
        TextInstantiator.InstantiateText(FirstNumPlace, "then", XPos + 400, Ypos, 0, false);
        TextInstantiator.InstantiateText(FirstNumPlace, "m<sub>1</sub>=m<sub>2</sub>", XPos + 650, Ypos, 0, false);
        Ypos -= 125;
